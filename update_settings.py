...
def update_workspace_file(workspace_file):
    """
    Updates the `.code-workspace` file with the correct interpreter path.
    """
    print(f"Checking workspace file: {workspace_file}")  # Debugging

    try:
        with open(workspace_file, "r") as file:
            workspace = json.load(file)
            print(f"Current workspace settings: {workspace.get('settings', {})}")  # Debugging
