...
class MetricsTrackerSingleton:
    ...
    def reset(self):
        ...
        self.skipped_dict = {}  # {item_identifier: id_for_skipped_item}
    ...
    def print_interim_summary(self):
        """Print current statistics without ending the tracking."""
        if self.start_time is None:
            print("Processing hasn't started yet.")
            return
        ...
        if items_processed == 0:
            print(f"No {self.object_name}s processed yet.")
            return
        
        print(f"\n--- Interim Summary of {self.object_name} processing ---")
        print(f"\tCurrent {self.object_name}s processed: {items_processed} with {self.processed_count} successful.")
        
        if self.total_to_process is not None and self.total_to_process > 0:
            progress_percent = (items_processed / self.total_to_process) * 100
            print(f"\tProgress: {progress_percent:.1f}% ({items_processed}/{self.total_to_process})")
            print(f"\tRemaining {self.object_name}s: {self.total_to_process - items_processed}")
        
        print(f"\tCurrent {self.object_name}s skipped: {self.skipped_count}")
        print(f"\tCurrent {self.object_name}s with errors: {self.error_count}")
        print(f"\tCurrent {self.object_name}s failed: {self.failed_count}")
        
        # Per-item statistics
        time_per_item = total_time / items_processed
        print(f"\tAverage time per {self.object_name}: {time_per_item:.2f} seconds")
        
        # If total time > 2 minutes, show approximate items per minute
        if total_time > 120:
            ipm = items_processed / (total_time / 60.0)
            print(f"\tProcessing rate: {ipm:.0f} {self.object_name}s per minute")
        
        # Print elapsed time
        minutes, seconds = divmod(total_time, 60)
        if minutes:
            print(f"\tElapsed time: {int(minutes)} minute(s) {seconds:.2f} seconds")
        else:
            print(f"\tElapsed time: {seconds:.2f} seconds")
        
        print("--- End of Interim Summary ---\n")
    ...
    def end(self):
        ...
        if self.total_to_process is not None and self.total_to_process > 0:
            print(f"\tTotal {self.object_name}s not processed: {self.total_to_process - self.processed_count}")
        else:
            print(f"\tTotal {self.object_name}s not processed: {self.skipped_count + self.failed_count}")
        print(f"\tTotal {self.object_name}s skipped: {self.skipped_count}")
        print(f"\tTotal {self.object_name}s with errors: {self.error_count}")
        print(f"\tTotal {self.object_name}s failed: {self.failed_count}")
    ...
    def increment_processed_count(self, increment=1):
        """Increment the count for successfully processed items."""
        self.processed_count += increment


    def increment_error_count(self, item_identifier=None, increment=1):
        """
    ...
    def increment_failed_count(self, item_identifier=None, failure_info=None):
        """
    ...
    def increment_skipped_count(self, item_identifier=None, skipped_info=None):
        """
        Increment the count for items that failed completely.
        You can store a reason or exception message in failures_dict.
        """
        ...
        if item_identifier:
            # You could store just a count or a reason.
            # For demonstration, let's store the reason or a default message.
            self.skipped_dict[item_identifier] = (
                skipped_info if skipped_info else "Unknown skipped reason"
            )
