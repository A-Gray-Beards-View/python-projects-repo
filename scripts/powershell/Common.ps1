...
$workspaceFile = Get-ChildItem -Path $repoRoot -Filter "*.code-workspace" -Depth 0 | Select-Object -First 1
...
function Update-Requirements-Txt {
    ...
    try {
        ...
        if (-Not $isRoot) {
            Add-Content -Path requirements.txt -Value "-e ../repo_packages"
            Add-Content -Path requirements.txt -Value "-e ./workspace_packages"
        } else {
            Add-Content -Path requirements.txt -Value "-e ./repo_packages"
        }
